...
以太坊堆栈对不同层级的需求做了不同的抽象，并在不同层级之间交互来将以太坊融入项目。以太坊大致分为五个层级：

* **以太坊虚拟机**：以太坊虚拟机与执行机器之间的抽象层级，处理所有在以太坊网络执行的交易。以太坊虚拟机通过140个操作码使以太坊虚拟机图灵完备，只要资源足够就几乎可以计算一切东西。
* **智能合约**：在以太坊区块链上运行的可执行程序，为用户和程序之间交互提供接口。
* **以太坊节点**：应用程序可以通过连接以太坊节点读取以太坊区块链数据，发布交易。以太坊节点也是以太坊客户端，是以太坊的一种实现，可以验证交易，保证数据准确和网络安全。
* **以太坊客户端应用程序接口**：对于不同语言有丰富的库来实现连接终端应用程序与以太坊区块链。这些接口大大减少了与以太坊节点互动的复杂性。
* **终端用户应用程序**：面向用户的应用程序。

对于有不同需求的开发者考虑使用不同的技术栈实现自己想要的功能。
