...
## SHA256
### 常量与基本运算
通过8+64个初始常量，信息预处理，逻辑运算，轮密钥加实现。
#### 常量
**8个初始常量**：（8*8*4=256bits）$h_0~h_7$，通过自然数前8个指数的平方根小数部分的前32bit。

**64个常量：** 对自然数前64个指数的立方根的小数部分的前32bit。

#### 基本运算
有六个逻辑运算
* 前四个用于64轮循环（数据非线性压缩
* 后2个用于16个字落站为64个字（非线性扩展）
...
$$
$$
\Sigma0(x) = S^2(x)\oplus S^{13}(x)\oplus S^{22}(x)
$$
$$
\Sigma1(x) = S^6(x)\oplus S^{11}(x)\oplus S^{25}(x)
$$
$$
\sigma0(x) = S^7(x)\oplus S^{18}(x)\oplus R^3(x)
$$
$$
\sigma1(x) = S^{17}(x)\oplus S^{19}(x)\oplus R^{10}(x)
$$
...
### 数据预处理
在原信息后填充一个1，然后都补0 。在最后补充64bit数据长度，最后要填充到512bit。

### 计算摘要
首先将进入的消息分为512bit的块，整个消息M被分为n个块就需要n次迭代，最终结果就是256bit的数字摘要。

一个256bit的摘要初始值是H0，经过每一个数据块处理映射得到Hi，最后得到Hn。H0的初始值就是前面的8个初始常量。
### 映射函数
#### 扩展函数
输入512bit，把16个字扩展为64个字。剩下的48个字通过公式迭代得到：
$$
W_t = \sigma_1(W_{t-2}) + W_{t-7} + \sigma0(W_{t-15}) + W_{t-16}
$$
#### 压缩函数
对于产生的64字输入和64个常量，对H0做64次迭代，每次迭代的方法如图：

<img src="./imgs/SHA256.png" alt="SHA256迭代图" width="" height="" />

其中红色田字格是相加后$mod 2^{32}$。最后一次循环产生的八个字合起来就是输入对应的哈希。
